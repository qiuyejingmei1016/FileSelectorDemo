 ...
 */
public class PickerSelectionManager {

    private static volatile PickerSelectionManager mSelectionManager;

    private ArrayList<String> mSelectImagePaths = new ArrayList<>();

    private int mMaxCount = 1;

    private PickerSelectionManager() {
    }

    public static PickerSelectionManager getInstance() {
        if (mSelectionManager == null) {
            synchronized (PickerSelectionManager.class) {
                if (mSelectionManager == null) {
                    mSelectionManager = new PickerSelectionManager();
                }
            }
        }
    ...
    public static boolean isCanAddSelectionPaths(String currentPath, String filePath) {
        if ((PickerMediaFileUtil.isVideoFileType(currentPath) && !PickerMediaFileUtil.isVideoFileType(filePath)) || (!PickerMediaFileUtil.isVideoFileType(currentPath) && PickerMediaFileUtil.isVideoFileType(filePath))) {
            return false;
        }
        return true;
    }
